...
class schema extends arion
{
    ...
    public function up($sub_argv)
    {
        global $_APP;

        // sub --arguments
        if (in_array('mute', $sub_argv)) $this->mute = true;
            ...
            for ($y = 0; $y < count($wild_loop); $y++) {
                ...
                for ($i = 0; $i < count($tables); $i++) {
                    ...
                    if (is_file($fp)) {
                        ...
                        // convert fields
                        $field = $this->convertField(@$data['field']);
                        ...
                        $field_curr = array();
                        // table exists?
                        if (in_array($table, $tables_real)) {
                            $r = $my->query("SHOW COLUMNS FROM $table");
                            if ($r[0]) {
                                for ($x = 0; $x < count($r); $x++) $field_curr[$r[$x]['Field']] = $r[$x];
                                //pre($field_curr);
                                $this->updateTable($table, $field, $field_curr, $my);
                            }
                        }
                        // table dont exists
                        else $this->createTable($table, $field, $my);
                    }
                    nextTable:
                }
    ...
    private function convertField($field)
    {
        // create new fields
        $new_field = array();
        if (!@$field[0]) goto convertFieldEnd;
