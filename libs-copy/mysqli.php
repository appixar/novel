...
function jwconex($i = 0)
{
    ...
    debug(__FUNCTION__, "(#$i) connecting to {$c['HOST']} {$c['PORT']} ({$c['NAME']})...", "sql_conex");
    $jwconex[$i] = @mysqli_connect($c['HOST'], $c['USER'], $c['PASS'], $c['NAME'], $c['PORT']);
    if (mysqli_connect_error()) {
        debug(__FUNCTION__, mysqli_connect_error(), "error");
        arion::err("DB ERROR = CAN'T CONNECT.", mysqli_connect_error());
    } else {
        $time_elapsed_secs = number_format((microtime(true) - $start), 4);
        debug(__FUNCTION__, "(#$i) connected in $time_elapsed_secs s", "sql_conex");
    }
}
...
function jwquery($query, $i = 0)
{
    ...
    if ($jwconex[$i]) {
        ...
        debug(__FUNCTION__, "(#$i) $query", "sql_sel");
        $rs = mysqli_query($jwconex[$i], $query) or debug(__FUNCTION__, mysqli_error($jwconex[$i]), "error");
        if (!$rs) {
        ...
        $time_elapsed_secs = number_format((microtime(true) - $start), 4);
        debug(__FUNCTION__, "$rows_aff rows affected in $time_elapsed_secs s", "sql_sel");
    }
    if (isset($rs)) {
        if (is_bool($rs)) {
            return $rs;
...
function jwinsert($table_name, $array_data, $i = 0)
{
    ...
    $start = microtime(true);
    debug(__FUNCTION__, "(#$i) $qr", "sql_ins");

    $rs = mysqli_query($jwconex[$i], $qr) or debug(__FUNCTION__, mysqli_error($jwconex[$i]), "error");
    if (!$rs) {
        #die(mysqli_error($jwconex[$i]));
    ...
    $rows_aff = mysqli_affected_rows($jwconex[$i]);
    $time_elapsed_secs = number_format((microtime(true) - $start), 4);
    debug(__FUNCTION__, "$rows_aff rows affected in $time_elapsed_secs s", "sql_ins");
...
function jwupdate($table_name, $array_data, $array_condition, $i = 0)
{
    ...
    $start = microtime(true);
    debug(__FUNCTION__, "(#$i) $qr", "sql_upd");
    ...
    $rows_aff = mysqli_affected_rows($jwconex[$i]);
    $time_elapsed_secs = number_format((microtime(true) - $start), 4);
    debug(__FUNCTION__, "$rows_aff rows affected in $time_elapsed_secs s", "sql_upd");
    return $rows_aff;
}
//--------------
// PRE FUNCTION
